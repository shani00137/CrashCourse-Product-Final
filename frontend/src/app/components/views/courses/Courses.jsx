...
function CoursesScreen() {
  ...
  const load = useCallback(() => {
    ...
    setLoading(true);
    setError(null);
    getAllCourses({ pageNumber: page, pageSize, searchTerm: debouncedSearch }).then((res) => {
      if (cancelled) return;
      setRows(res.data ?? []);
  ...
  const typeBadge = (t) => t === "PDF" ? "bg-red-50 text-red-600" : t === "Video" ? "bg-blue-50 text-blue-600" : "bg-purple-50 text-purple-600";
  const materialTypeIcon = (t) => t === "PDF" ? <File size={12} /> : t === "Video" ? <Video size={12} /> : <Mic size={12} />;
  ...
  >
    ...
    /* Add / Edit modal */
      ...
      {formOpen && <Modal title={editing ? "Edit Course" : "Add New Course"} onClose={() => setFormOpen(false)}>
    ...
    /* Attachment modal */
      ...
      {materialCourse && <Modal title={`Attachments \u2014 ${materialCourse.courseName}`} onClose={() => setMaterialCourse(null)} className="max-w-2xl">
          <div className="flex flex-col gap-4">
            <div className="overflow-x-auto rounded-lg border border-[rgba(0,0,0,0.06)]">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-[rgba(0,0,0,0.06)] bg-[#F7FAFC]">
                    {["File Name", "Type", "Actions"].map((h) => <th key={h} className="text-left px-4 py-2.5 text-[11px] font-semibold text-[#718096] uppercase tracking-wide">{h}</th>)}
                  </tr>
                </thead>
                <tbody>
                  {materials.map((m) => <tr key={m.courseMaterialId} className="border-b border-[rgba(0,0,0,0.04)] hover:bg-[#F7FAFC] transition-colors">
              ...
              {!materialsLoading && materialsError && <div className="py-6 text-center">
                  <AlertCircle size={22} className="mx-auto text-red-400 mb-2" />
                  <p className="text-sm text-red-600 font-medium">{materialsError}</p>
                </div>}
              {!materialsLoading && !materialsError && materials.length === 0 && <div className="py-10 text-center">
  ...
  />
    ...
    /* Toast */
      ...
      {toast && <div className={`fixed top-5 right-5 z-[60] flex items-center gap-2 px-4 py-3 rounded-xl shadow-lg text-sm font-medium text-white ${toast.type === "success" ? "bg-emerald-500" : "bg-red-500"}`}>

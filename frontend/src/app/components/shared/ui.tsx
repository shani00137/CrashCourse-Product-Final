...
// ─── Button ───────────────────────────────────────────────────────────────────
export function Btn({ children, variant = "primary", onClick, className = "", icon, disabled, type = "button" }: {
  children?: ReactNode; variant?: "primary" | "secondary" | "danger" | "ghost" | "outline";
  onClick?: () => void; className?: string; icon?: ReactNode; disabled?: boolean; type?: "button" | "submit";
}) {
  const styles = {
...
export function BouncingDots({ label = "Searching…", color = "#0E7C7B" }: { label?: string; color?: string }) {
  return (
...
export function SearchableSelect({ options, value, onSelect, allLabel, placeholder }: {
        ...
        <div className="absolute z-30 mt-1 w-64 bg-white rounded-lg border border-[rgba(0,0,0,0.12)] shadow-xl">
          <div className="p-2 border-b border-[rgba(0,0,0,0.06)]">
            <div className="relative">
              ...
              />
              {query && (
                <button onClick={() => setQuery("")} className="absolute right-1.5 top-1/2 -translate-y-1/2 p-0.5 text-gray-400 hover:text-gray-600">
                  <X size={12} />
                </button>
              )}
            </div>
          ...
          <div className="max-h-56 overflow-y-auto py-1">
            ...
            {filtered.length === 0 && (
              <p className="px-3 py-2 text-xs text-gray-400">No matches for "{query}"</p>
            )}
            {filtered.map(o => (
              <button
                key={o.id}
...
// ─── Modal ────────────────────────────────────────────────────────────────────
export function Modal({ title, children, onClose, className = "max-w-lg" }: {

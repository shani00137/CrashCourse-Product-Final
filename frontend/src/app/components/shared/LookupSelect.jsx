  ...
  return (
    <div className={`flex flex-col gap-1 ${className}`} ref={ref}>
      ...
      <button
        ...
        className={`h-9 px-3 rounded-lg border bg-white text-sm flex items-center justify-between gap-2 min-w-44 transition ${disabled ? "opacity-60 cursor-not-allowed" : open ? "border-[#C41E3A] ring-1 ring-[#C41E3A]" : "border-[rgba(0,0,0,0.12)] hover:border-[#C41E3A]"} ${required ? "border-red-300" : ""}`}
      >
  ...
  return (
    <div className={`flex flex-col gap-1 relative ${className}`} ref={ref}>
      ...
      <button
        ...
        onClick={() => !disabled && setOpen((o) => !o)}
        disabled={disabled}
        className={`h-10 px-3 rounded-lg border bg-white text-sm flex items-center justify-between gap-2 transition ${disabled ? "opacity-60 cursor-not-allowed" : open ? "border-[#C41E3A] ring-1 ring-[#C41E3A]" : "border-[rgba(0,0,0,0.12)] hover:border-[#C41E3A]"}`}
      >
      ...
      {open && !disabled && (
        <div className="absolute z-30 mt-1 w-72 bg-white rounded-lg border border-[rgba(0,0,0,0.12)] shadow-xl top-full left-0">
          ...
          <div className="max-h-56 overflow-y-auto py-1">
            <button
              onClick={() => onChange?.([])}
              className={`w-full text-left px-3 py-1.5 text-sm transition ${value.length === 0 ? "bg-red-50 text-[#C41E3A] font-medium" : "text-[#1A202C] hover:bg-[#F7FAFC]"}`}
            >
              {allLabel}
            </button>
            {filtered.length === 0 && q && <p className="px-3 py-2 text-xs text-gray-400">No matches for "{debouncedQuery}"</p>}
            {filtered.map((o) => (
              <label key={o.id} className="flex items-center gap-2 w-full text-left px-3 py-1.5 text-sm cursor-pointer hover:bg-[#F7FAFC] transition">
                <input type="checkbox" checked={value.includes(o.id)} onChange={() => toggle(o.id)} className="accent-[#C41E3A]" />
                <span className="truncate flex-1">{o.label}</span>
                {o.price != null && (
                  <span className="text-xs font-mono text-[#C41E3A] whitespace-nowrap">{Number(o.price).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                )}
              </label>
            ))}
          </div>
      ...
      {selected.length > 0 && (
        <div className="flex flex-wrap gap-1">
          {selected.map((o) => (
            <span key={o.id} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-red-50 text-[#C41E3A] text-xs font-medium">
              {o.label}
              {o.price != null && <span className="font-mono text-[#C41E3A]">{Number(o.price).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>}

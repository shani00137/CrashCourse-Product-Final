...
namespace IMS_WebApp.Controllers
{
    public class UserPermissionController : ApiController
    {
        ...
        public IHttpActionResult GetManuePages(int id)
        {
            ...
            var query = (from c in db.UserPermissions
                         ...
                         select new
                         {
                             ...
                             PageName=(from c in db.UserPages join w in db.UserPermissions on c.PageId equals w.PageId
                                       join m in db.UserInfoes on w.RoleId equals m.RoleId
                                       where  c.MenuId == g.Key.MenuId && m.UserNo==id select c).ToList()
        ...
        public IHttpActionResult GetUserControllers()
        {
            List<UserPermissionMD> Userlist = new List<UserPermissionMD>();
            MdLabScienceDbEntities db = new MdLabScienceDbEntities();
            var query = (from c in db.UserControllers join w in db.UserMenus on c.MenuId equals w.MenuId select new {
        ...
        public IHttpActionResult GetAllUserPermissions(int id)
        {
